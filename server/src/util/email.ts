import { IEmailJSSendBody, IEmailRequestBody } from "../types/email.js";
import {
  EMAILJS_PRIVATE_KEY,
  EMAILJS_PUBLIC_KEY,
  EMAILJS_TEMPLATE_ID,
...
const SWEAR_WORDS = fs.readFileSync(SWEAR_WORDS_DATA_PATH, "utf-8").split("\n");
...
export async function sendEmail(emailBody: IEmailRequestBody): Promise<void> {
  ...
  if (
    finalSenderEmail.length < MINIMUM_EMAIL_LENGTH ||
    finalSenderEmail.length > MAXIMUM_EMAIL_LENGTH
  ) {
    throw new Error(
      `${finalSenderEmail} is not between ${MINIMUM_EMAIL_LENGTH} and ${MAXIMUM_EMAIL_LENGTH} characters long`,
    );
  }
  ...
  if (
    senderName.length < MINIMUM_NAME_LENGTH ||
    senderName.length > MAXIMUM_NAME_LENGTH
  ) {
    throw new Error(
      `${senderName} is not between ${MINIMUM_NAME_LENGTH} and ${MAXIMUM_NAME_LENGTH} characters long`,
    );
  }
  ...
  if (
    messageBody.length < MINIMUM_MESSAGE_BODY_LENGTH ||
    messageBody.length > MAXIMUM_MESSAGE_BODY_LENGTH
  ) {
    throw new Error(
      `${messageBody} is not between ${MINIMUM_MESSAGE_BODY_LENGTH} and ${MAXIMUM_MESSAGE_BODY_LENGTH} characters long`,
    );
  }
  ...
  if (SERVER_ENV !== "development") {
    ...
    if (!response.ok) {
      throw new Error(
        `The EmailJS server responded with a status code of ${response.status}`,
      );
    }
  }
}
